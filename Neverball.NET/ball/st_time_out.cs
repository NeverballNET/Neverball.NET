...
namespace Neverball.NET
{
    class st_time_out
    {
        public const int TIME_OUT_NEXT = 1;
        public const int TIME_OUT_SAME = 2;
        public const int TIME_OUT_BACK = 4;
        public const int TIME_OUT_OVER = 5;


        public static int time_out_action(int i)
        {
        ...
        public static State get_st_time_out()
        {
            if (st_time_out_class.Instance == null)
            {
                st_time_out_class.Instance = new st_time_out_class();
            }

            return st_time_out_class.Instance;
        }
    }


    class st_time_out_class : State
    {
        public static st_time_out_class Instance = null;
        ...
        internal override int OnEnter()
        {
            ...
            {
                kd = gui.gui_label(id, "Time's Up!", gui.GUI_LRG, gui.GUI_ALL, widget.gui_gry, widget.gui_red);
                ...
                if ((jd = gui.gui_harray(id)) != 0)
                {
                    if (Progress.progress_dead() != 0)
                        gui.gui_start(jd, "Exit", gui.GUI_SML, st_time_out.TIME_OUT_OVER, 0);

                    if (Progress.progress_next_avail() != 0)
                        gui.gui_start(jd, "Next Level", gui.GUI_SML, st_time_out.TIME_OUT_NEXT, 0);

                    if (Progress.progress_same_avail() != 0)
                        gui.gui_start(jd, "Retry Level", gui.GUI_SML, st_time_out.TIME_OUT_SAME, 0);
                }
                gui.gui_space(id);
        ...
        internal override int OnButton(int b, int d)
        {
            if (d != 0)
            {
                if (Config.config_tst_d(Config.CONFIG_JOYSTICK_BUTTON_A, b) != 0)
                    return st_time_out.time_out_action(gui.gui_token(gui.gui_click()));
                if (Config.config_tst_d(Config.CONFIG_JOYSTICK_BUTTON_EXIT, b) != 0)
                    return st_time_out.time_out_action(st_time_out.TIME_OUT_BACK);
            }
            return 1;
        }


        public st_time_out_class()
        {
            pointer = 1;
            gui_id = 0;
        }
