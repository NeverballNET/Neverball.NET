...
namespace Neverball.NET
{
    class s_jump
    {
        ...
        public float[] m_q = new float[3];                                /* target position            */
        public float m_r;                                   /* radius                     */

        static int jump_init_n = 0;
        ...
        public static void jump_init(int b)
        {
            jump_init_n = b != 0 ? 32 : 8;
        }


        static int jump_list;
        void jump_list_draw(int index)
        {
            if (jump_list == 0)
            {
                jump_list = Video.GenLists(2);

                for (int k = 0; k < 2; k++)
                {
                    Video.NewList(jump_list + k, Video.COMPILE);
                    {
                        Video.Begin(Video.QUAD_STRIP);
                        {
                            for (int i = 0; i <= jump_init_n; i++)
                            {
                                float x = (float)System.Math.Cos(MathHelper.TwoPi * i / jump_init_n);
                                float y = (float)System.Math.Sin(MathHelper.TwoPi * i / jump_init_n);

                                Video.Color(0.75f, 0.5f, 1.0f, (k == 0 ? 0.5f : 0.8f));
                                Video.Vertex3(x, 0.0f, y);

                                Video.Color(1.0f, 1.0f, 1.0f, 0.0f);
                                Video.Vertex3(x, Config.JUMP_HEIGHT, y);
                            }
                        }
                        Video.End();
                    }
            ...
            {
                Video.CallList(jump_list + index);
            }
        }

        public void jump_draw(int highlight)
        {
