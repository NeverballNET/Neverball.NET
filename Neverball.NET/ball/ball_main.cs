...
namespace Neverball.NET
{
    ...
    {
        static int t0, uniform;

        public static int Init()
        {
            State.Current = StateNull.Instance;

            /* Initialise demo playback. */

            State.goto_state(st_title.st_ball_title());

            /* Run the main game loop. */

            uniform = Config.config_get_d(Config.CONFIG_UNIFORM);
            t0 = (int)Alt.IntervalTimer.TimeMSec;
        ...
        {
            int t1 = (int)Alt.IntervalTimer.TimeMSec;
            ...
            if (uniform != 0)
            {
                ...
                for (u = 0; u < System.Math.Abs//abs
                    (uniform); ++u)
                {
                    State.st_timer(Config.DT);
                    t0 += (int)(Config.DT * 1000);
                }
            }
            else
            {
